...
- **🎯 优化评论命令**: `Add Comment Inline` 命令现在提供真正的内联体验
- **🚀 性能提升**: 重构内联聊天组件，提高响应速度和稳定性
- **📝 简化用户界面**: 移除冗余提示文本，提供更清爽的用户体验

### Technical Improvements

- 完全重写 `InlineChatWidget` 类，使用VS Code原生API
- 优化资源管理和内存清理
- 改进错误处理和用户反馈机制
...
- **💬 评论界面优化**: 新增Copilot风格的内联评论输入框，直接显示在代码上方
- **🔍 多行选择支持**: 修复多行代码选择时只有一行被标记为评论的问题
- **👁️ 用户体验改善**: 评论输入界面更符合直觉，提供更好的视觉反馈

### Fixed

- 修复添加评论时选择多行但只高亮一行的问题
- 优化代码镜头放置位置，使其始终显示在选择的起始行
